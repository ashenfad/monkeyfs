...
|--------|-----------|
| `builtins` / `io` | `open` |
| `os` | `listdir`, `scandir`, `remove`, `unlink`, `mkdir`, `makedirs`, `rmdir`, `rename`, `replace`, `stat`, `lstat`, `getcwd`, `chdir`, `utime`, `getenv`, `access`, `readlink`, `symlink`, `link`, `chmod`, `chflags` (BSD/macOS), `chown`, `truncate`, `listxattr`, `getxattr`, `setxattr`, `removexattr` (Linux), `open`, `read`, `write`, `close`, `fstat`, `lseek` |
