...
| `pathlib` | `Path.touch`; `_NormalAccessor` (3.10) -- every captured reference on it: `stat`, `lstat`, `scandir`, `open`, `unlink`, `rmdir`, `rename`, `mkdir`, `listdir`, `getcwd`, `chmod`, `replace`, `link`, `symlink`, `readlink`, `expanduser`, `realpath`; `Path._globber` (3.13+) |
